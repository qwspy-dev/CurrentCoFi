import { Current, type CurrentRecipient } from "@currentcofi/sdk";
...
function failure(code: string, message: string) {
  return { isError: true, content: [{ type: "text" as const, text: JSON.stringify({ ok: false, error: { code, message } }, null, 2) }] };
}

export function configFromEnv(env: NodeJS.ProcessEnv = process.env): CurrentMcpConfig {
  return {
...
export function createCurrentMcpServer(input: CurrentMcpConfig = configFromEnv()) {
  ...
  const server = new McpServer(
    { name: "current-cofi", version: "0.1.0" },
    { instructions: "Use Current's public proof tools before recommending an integration. Project mutations require explicit user approval, a stable idempotency key, and scoped credentials. A proposed reward is not settled until Current returns completed with Arc transaction evidence." },
  );

  async function publicGet<T>(path: string) {
    const response = await request(`${baseUrl}${path}`, { headers: { accept: "application/json" } });
    const envelope = await response.json() as Envelope<T>;
    if (!response.ok || !envelope.ok || envelope.data === undefined) throw new Error(envelope.error?.message ?? `Current returned HTTP ${response.status}.`);
    return envelope.data;
  }

  function requireProject() {
    if (!current) throw new Error("Project mode requires CURRENT_MCP_MODE=project, CURRENT_API_KEY, and CURRENT_SIGNING_SECRET.");
    return current;
  }
  ...
  server.registerTool("current_get_campaign_analytics", {
    ...
    try { return success(await requireProject().analytics.get()); }
  ...
  server.registerTool("current_list_campaign_deliveries", {
    ...
    try { return success(await requireProject().deliveries.list(distributionId)); }
    catch (error) { return failure("PROJECT_MODE_REQUIRED", error instanceof Error ? error.message : "Campaign deliveries are unavailable."); }
  });
  ...
  server.registerTool("current_record_campaign_handoff", {
    ...
    void _approval;
    try { return success(await requireProject().deliveries.recordHandoff(deliveryId, channel)); }
    catch (error) { return failure("CURRENT_ACTION_REJECTED", error instanceof Error ? error.message : "Current rejected the delivery handoff."); }
  });
  ...
  server.registerTool("current_list_community_bounties", {
    ...
    try { return success(await requireProject().bounties.list()); }
    catch (error) { return failure("PROJECT_MODE_REQUIRED", error instanceof Error ? error.message : "Project bounties are unavailable."); }
  });
  ...
  server.registerTool("current_create_community_bounty", {
    ...
    void _approval;
    try { return success(await requireProject().bounties.create(value)); }
  ...
  server.registerTool("current_list_verifiable_giveaways", {
    ...
    try { return success(await requireProject().giveaways.list()); }
  ...
  server.registerTool("current_create_verifiable_giveaway", {
    ...
    void _approval;
    try { return success(await requireProject().giveaways.create(value)); }
    catch (error) { return failure("CURRENT_ACTION_REJECTED", error instanceof Error ? error.message : "Current rejected the giveaway."); }
  });
  ...
  server.registerTool("current_list_launch_vesting", {
    ...
    try { return success(await requireProject().vesting.list()); }
    catch (error) { return failure("PROJECT_MODE_REQUIRED", error instanceof Error ? error.message : "Launch vesting is unavailable."); }
  });
  ...
  server.registerTool("current_create_launch_vesting", {
    ...
    void _approval;
    try { return success(await requireProject().vesting.create(value)); }
    catch (error) { return failure("CURRENT_ACTION_REJECTED", error instanceof Error ? error.message : "Current rejected the vesting batch."); }
  });
  ...
  server.registerTool("current_propose_reward_distribution", {
    ...
    void _approval;
    try { return success(await requireProject().agentActions.proposeDistribution({ ...value, recipients: recipients as CurrentRecipient[] })); }
    catch (error) { return failure("CURRENT_ACTION_REJECTED", error instanceof Error ? error.message : "Current rejected the proposal."); }
  });
  ...
  server.registerTool("current_submit_activation", {
    ...
    void _approval;
    try { return success(await requireProject().activations.submit({ ...value, walletAddress: value.walletAddress as `0x${string}` })); }
