...
export function createCurrentMcpServer(input = configFromEnv()) {
    ...
    server.registerTool("current_get_programmable_commerce", {
        ...
        try {
            return success(await requireProject().checkout.list());
        }
        catch (error) {
            return failure("PROJECT_MODE_REQUIRED", error instanceof Error ? error.message : "Programmable commerce is unavailable.");
        }
    });
    server.registerTool("current_propose_programmable_checkout", {
        ...
        void _approval;
        const splits = value.splits?.map((split) => ({ ...split, recipientAddress: split.recipientAddress }));
        try {
            return success(await requireProject().agentActions.proposeCheckout({ ...value, splits, kind: "programmable_checkout" }));
        }
        catch (error) {
            return failure("CURRENT_ACTION_REJECTED", error instanceof Error ? error.message : "Current rejected the checkout proposal.");
        }
    });
    ...
    server.registerTool("current_list_public_mass_drops", {
        ...
        try {
            return success(await requireProject().drops.list());
        }
        catch (error) {
            return failure("PROJECT_MODE_REQUIRED", error instanceof Error ? error.message : "Public mass drops are unavailable.");
        }
    });
    server.registerTool("current_create_public_mass_drop", {
        ...
        void _approval;
        try {
            return success(await requireProject().drops.create(value));
        }
        catch (error) {
            return failure("CURRENT_ACTION_REJECTED", error instanceof Error ? error.message : "Current rejected the public mass drop.");
        }
    });
