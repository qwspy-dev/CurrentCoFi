...
const campaignAbi = parseAbi(["function campaigns(bytes32) view returns (address sender,address token,uint128 totalAmount,uint128 remainingAmount,uint64 expiresAt,uint32 recipientCount,bytes32 merkleRoot,uint8 state)", "function isClaimed(bytes32,uint256) view returns (bool)"]);
const multicall = "0xcA11bde05977b3631167028862bE2a173976CA11" as Address;
const settlementCampaignId = "0x2c4f2b00ce218f40f044eeea0a96dcea16cd53bc122d3fb6637fd4c2ad6f7e8b" as const;
...
export async function getPartnerVaultSnapshot() {
  ...
  if (!vault || !governor || !token) return { configured: false, network: ARC_TESTNET.network, addresses: null, asset: null, governance: null, proofCampaign: null, settlementProof: null };
  ...
  await new Promise((resolve) => setTimeout(resolve, 1_100));
  const [settlementCampaignValue, settlementClaimedValue] = await client.multicall({ multicallAddress: multicall, allowFailure: false, contracts: [
    { address: values[1] as Address, abi: campaignAbi, functionName: "campaigns", args: [settlementCampaignId] },
    { address: values[1] as Address, abi: campaignAbi, functionName: "isClaimed", args: [settlementCampaignId, BigInt(0)] },
  ] as never }) as readonly unknown[];
  const settlement = settlementCampaignValue as readonly [Address, Address, bigint, bigint, bigint, number, `0x${string}`, number];
  return {
    ...
    settlementProof: {
      id: String(settlementCampaignId), token: settlement[1], totalAmount: formatUnits(settlement[2], decimals), remainingAmount: formatUnits(settlement[3], decimals),
      expiresAt: Number(settlement[4]), recipientCount: Number(settlement[5]), merkleRoot: settlement[6], state: Number(settlement[7]), claimed: Boolean(settlementClaimedValue),
    },
