...
Project mode adds authenticated analytics and two write tools. Store credentials in the agent host's secret manager, never in prompts or source control.
...
The integration test launches the compiled server over stdio, connects through the official MCP client, discovers all seven tools, exercises public and authenticated reads, verifies signed write requests, and proves that missing approval is rejected.
