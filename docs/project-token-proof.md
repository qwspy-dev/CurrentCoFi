...
- `/#/project-token-proof` presents the asset, reserve, delayed governance, funded campaign, contracts, and transactions in an account-free reviewer interface.
...
5. Public contract state and transaction anchors agree with the presented record.

## Honest boundary

CPT has no monetary value and implies no external partner endorsement. The proof demonstrates protocol capability; it is not included in Network Proof's persisted user-traction totals, and it does not claim a completed recipient settlement. That distinction remains explicit in the API, reviewer UI, and grant dossier.
